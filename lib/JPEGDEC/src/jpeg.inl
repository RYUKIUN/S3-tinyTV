...
void s3_idct_columns(int16_t *pMCU, const int16_t *pConst);
}
int16_t i16_Consts[8] = {0x80, 113, 90, 22, 46, 1,32,2048};
// Column-pass IDCT butterfly constants for s3_idct_columns(), 14-bit fixed
// point (13-bit for the one entry that would otherwise overflow int16) -
// see s3_simd_idct_columns.S header comment and scratchpad/idct_columns_
// validate.c for derivation and correctness validation against the scalar
// reference. s3_idct_columns expects pMCU already dequantized (call
// s3_dequant first) and only replaces the column-pass butterfly; row pass
// and pixel clamp stay scalar (PIE has no shuffle/transpose/gather, which
// both of those stages would need).
static const int16_t __attribute__((aligned(16))) s3_idct_pConst[32] = {
    23170,23170,23170,23170,23170,23170,23170,23170, // K1414  = round(1.414213562 * 2^14)
    30274,30274,30274,30274,30274,30274,30274,30274, // K1847  = round(1.847759065 * 2^14)
    -21407,-21407,-21407,-21407,-21407,-21407,-21407,-21407, // KNEG2613 = round(-2.613125930 * 2^13)
    17735,17735,17735,17735,17735,17735,17735,17735, // K10824 = round(1.082392200 * 2^14)
};
// Kill switch: comment out (or pass -DIDCT_USE_SIMD=0) to fall back to the
// pure-scalar column pass for A/B comparison, same pattern as TOUCH_USE_IRQ
// in the main firmware. Default OFF until validated on real hardware.
#ifndef IDCT_USE_SIMD
#define IDCT_USE_SIMD 0
#endif
#endif // S3 SIMD
#endif // __has_include
#endif // ESP32
...
#if defined(ESP32S3_SIMD) && IDCT_USE_SIMD
    // Column-pass butterfly on PIE, all 8 columns at once. Always takes the
    // "full calculation" path (no shortcut for an all-zero bottom half) -
    // see s3_simd_idct_columns.S for why. Row pass below is unaffected and
    // still reads pMCUSrc exactly as it would after the scalar column loop.
    s3_dequant(pMCUSrc, pQuant);
    s3_idct_columns(pMCUSrc, s3_idct_pConst);
#else
    // do columns first
...
#endif // ESP32S3_SIMD && IDCT_USE_SIMD
#endif // NO SIMD
    // now do rows
    u16MCUFlags = pJPEG->u16MCUFlags;
    pOutput = (unsigned char *)pMCUSrc; // store output pixels back into MCU
