...
- **Touchscreen drives the PC** — the XPT2046 panel acts as a remote input device for the machine it's mirroring: **tap** to left-click wherever you touched, **slide** to scroll, **hold still then move** to press-and-drag (for dragging windows around). Calibration is a 16-point bilinear least-squares fit that reports its own accuracy and redoes itself if a tap landed badly. Windows has only one system cursor, so a touch has to move it — but the cursor is *borrowed, not stolen*: it snaps back to wherever your physical mouse left it as soon as the gesture ends, and stands down entirely if you grab the mouse mid-gesture. It shares the display's SPI bus, but the display always wins it: `displayTask` holds a priority-inheriting mutex across its entire DMA push, so touch can delay a frame by at most one 456 µs read, and only while a finger is down. The sampler polls at 20 Hz while idle (~0.9% bus occupancy); once the PENIRQ line is wired and `TOUCH_USE_IRQ` is flipped to 1 in `nexus.h`, it sleeps on the interrupt instead and an untouched panel costs zero CPU and zero SPI traffic. Events go out over the existing UDP socket at ~0.35% of the video stream's bandwidth. Toggle it live with the **Enable Touch** control in the PC UI.
